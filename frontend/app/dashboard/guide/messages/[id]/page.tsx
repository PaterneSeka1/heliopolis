...
export default function GuideChatPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
    ...
    <div className="flex flex-col h-screen max-h-screen">
          ...
          return (
            <div key={msg.id} className={`flex flex-col max-w-[78%] ${isMine ? 'self-end items-end' : 'self-start items-start'}`}>
              ...
              <div className={`px-3 py-2.5 rounded-[18px] text-sm leading-relaxed ${
                isMine
                  ? 'bg-gradient-to-br from-[#6A1B9A] to-[#4a1370] text-white rounded-br-[5px]'
      ...
      <div className="flex-shrink-0 bg-white border-t border-[#e6e6ea] px-2.5 py-2 flex items-center gap-1.5 pb-safe">
        ...
        <button
          onClick={sendMessage}
          disabled={!input.trim()}
          className="w-10 h-10 rounded-full bg-gradient-to-br from-[#6A1B9A] to-[#4a1370] flex items-center justify-center text-white text-base disabled:opacity-40"
        >
          ➤
        </button>
      </div>
