...
export default function DashboardRegionPage() {
  ...
  return (
    <div className="p-6">
      {/* Top bar */}
      <div className="px-0 py-0 flex justify-between items-center mb-5 border-b border-[#ececf0] pb-4 bg-transparent">
        <div>
      ...
      <div className="grid grid-cols-4 gap-3.5 mb-5">
        {[
          ...
          <div key={kpi.label} className="bg-white border border-[#ececf0] rounded-2xl p-4 relative overflow-hidden">
            ...
            <div className={`text-xs mt-1 font-semibold ${'neg' in kpi && kpi.neg ? 'text-[#C62828]' : 'text-[#2E7D32]'}`}>{kpi.delta}</div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white border border-[#ececf0] rounded-2xl p-4 mb-5">
        <h3 className="font-bold text-sm text-[#1F1B2E] mb-4 flex justify-between">
          ...
          <Link href="/dashboard/region/doyennes" className="text-xs text-[#C62828] font-semibold">Détail →</Link>
        </h3>
            ...
            {districts.map(district => {
              ...
              return (
                <tr key={district.id} className="border-b border-[#f0f0f4] hover:bg-[#fafafc]">
                  ...
                  <td className="px-3 py-3">
                    <Link href="/dashboard/region/doyennes"
